...
$query = "
		...
		AND (
			( `n`.`value` LIKE '%chain%' AND `n`.`value` NOT LIKE '%stainless steel%' AND ( `n`.`value` LIKE '%gold%' OR `n`.`value` LIKE '%platinum%' ) )
			OR
			( (`n`.`value` LIKE '%eternity%' OR `n`.`value` LIKE '%gold%' AND ( `n`.`value` LIKE '%ring%' OR `n`.`value` LIKE '%band%' )) OR `n`.`entity_id` IN (SELECT `product_id` FROM `catalog_category_product` WHERE `category_id` = 411) )
		)
...
////		if ( ( (stripos($product->getName(), 'chain')!==false) || (stripos($product->getName(), 'ring')!==false) ) ) { // && ($product->getTypeId()=='simple') ) {

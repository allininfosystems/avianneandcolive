...
jQuery(document).ready(function(){
	jQuery('.filter-main-group .filter-price').remove();
    var i=true;
    var j=true;
    var k="";
    var l="";
    ...
    function getCustomPriceRangeValues() {
    	max_price=parseInt(jQuery("input#max_price_fix_value").val());
    	var values = new Array();
    	var step = 0;
    	while (step<max_price) {
    		if (step<1000)
    			step+=100;
    		else if (step<5000)
    			step+=200;
    ...
    function StartSlider(){
        ...
        var slider = jQuery("#slider-range-price").slider({
            ...
            //values:values,
            slide:function(a,b){
                var value = findNearest(b.value);
                if (b.value == b.values[0]) {
                    slider.slider('values', 0, value);
                }
                else {
                    slider.slider('values', 1, value);
                }
                jQuery("input#price_maximum").val(slider.slider('values', 1));
                jQuery("input#price_minimum").val(slider.slider('values', 0));
            },
            change:function(a,b){
                ...
                if (b.value == b.values[0]) {
                	jQuery("input#price_maximum").val(slider.slider('values', 1));
                    jQuery("input#price_minimum").val(value);
                    //slider.slider('values', 0, value);
                }
                else {
                	jQuery("input#price_maximum").val(value);
                    jQuery("input#price_minimum").val(slider.slider('values', 0));
                    //slider.slider('values', 1, value);
                }
                
                new_url=ajaxListURL(jQuery("#price_slider_url").val())+"&price="+jQuery("input#price_minimum").val()+"-"+jQuery("input#price_maximum").val();
                setReload_nav(true);
    ...
    jQuery("#price-filter-button, .block-layered-nav #narrow-by-list a, .block-layered-nav .currently a, .block-layered-nav .actions a").live("click",function(){
        setReload_nav(true);
        setReload_compare(true);
        if(jQuery(this).attr("id")=="price-filter-button"){
            ...
            request_price_min=Math.floor(jQuery("input[name=price_minimum]").last().val()/step_val)*step_val;
            request_price_max=Math.ceil(jQuery("input[name=price_maximum]").last().val()/step_val)*step_val;
            new_url=ajaxListURL(jQuery("#price_slider_url").val())+"&price="+request_price_min+"-"+request_price_max;
// console.log(request_price_min, request_price_max, new_url);
            window.location.hash=hashUrl(new_url)
        }else{
    ...
    function loadAjaxProductsList(e,f,g,h){
        ...
        jQuery.get(e,{},function(a,b,c){
            if(b=="error"){
                ...
                if(d.find('.col-main').html().indexOf('There are no products matching the selection')>=0) {
                	alert('No products match your selection');
                	jQuery(".products-list-loader").remove();
                	return false;
                } else {
                ...
                jQuery('.catalog-listing .grid-row .item table').each(function(item){
    ...
    jQuery('#narrow-by-list dt span').live( "click", function(event) {
    	var el = jQuery(this).parent('dt').siblings('.filter-dropdown');
    	if(el.is(":visible")) {
    		el.hide().parent('dl').removeClass('shown');
    	} else {
    		el.show().parent('dl').addClass('shown');
    	}
    });
    jQuery("html").click(function(event) {
    	if(!jQuery(event.target).parents('dl').hasClass('shown')) {
    		jQuery('#narrow-by-list .filter-dropdown').hide();
    	}
    });
    /*jQuery('#narrow-by-list').live( "hover", function(event) {
    	if (event.type == 'mouseenter') {
//    		jQuery(this).children('.filter-dropdown').show(); 
    	} else {
    		jQuery(this).children('.filter-dropdown').hide();
    	}
    });*/
    /*jQuery('#narrow-by-list .filter-state').live( "hover", function() {
    	return false;
    });*/
});

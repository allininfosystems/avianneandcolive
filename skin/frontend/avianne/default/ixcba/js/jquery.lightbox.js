if(undefined===window.lightbox_script_exists) {
var lightbox_script_exists = 'yes';
(function($) {  
  ...
  $.extend({
    LightBoxObject: {
      ...
      overlay : {
        ...
        hide: function(callback) {
          ...
          this.transition = this.element.fadeTo(this.options.closeDuration, 0, $.proxy(function(){
			if (window['amazon_widget_top'] == undefined){}else{amazon_widget_top = null;}
      ...
      create: function(options) {
        ...
        this.esqueleto.lightbox       = $('<div class="'+this.options.name+' '+this.options.name+'-mode-image"><div class="'+this.options.name+'-border-top-left"></div><div class="'+this.options.name+'-border-top-middle"></div><div class="'+this.options.name+'-border-top-right"></div><a class="'+this.options.name+'-button-close" id="lightbox_close" href="#close"><span>Close</span></a><div class="'+this.options.name+'-navigator"><a class="'+this.options.name+'-button-left" href="#"><span>Previous</span></a><a class="'+this.options.name+'-button-right" href="#"><span>Next</span></a></div><div class="'+this.options.name+'-buttons"><div class="'+this.options.name+'-buttons-init"></div><a class="'+this.options.name+'-button-left" href="#"><span>Previous</span></a><a class="'+this.options.name+'-button-max" href="#"><span>Maximize</span></a><div class="'+this.options.name+'-buttons-custom"></div><a class="'+this.options.name+'-button-right" href="#"><span>Next</span></a><div class="'+this.options.name+'-buttons-end"></div></div><div class="'+this.options.name+'-background"></div><div class="'+this.options.name+'-html"></div><div class="'+this.options.name+'-border-bottom-left"></div><div class="'+this.options.name+'-border-bottom-middle"></div><div class="'+this.options.name+'-border-bottom-right"></div></div>');
      ...
      addevents: function() {
		...
		/*
		var resize_once = 0;
		var already_resize = 0;
		$(document).bind('click', $.proxy(function(event) {
			if(resize_once == 0) {
				if($("#ixcba-billing").hasClass('active') == true) {
					already_resize = 1;
					resize_once = 1;
					this.movebox();
					$(".jquery-lightbox-move").css('top', '50');
				}
			}else if(already_resize == 1) {
				if($("#ixcba-billing").hasClass('active') == false) {
					already_resize = 0;
					resize_once = 0;
					this.movebox();
					$(".jquery-lightbox-move").css('top', '50');
        ...
        if (type=='image') {
          ...
          if (type == 'inline') {
            ...
            this.animations.ajax = $.ajax({
              ...
              cache    : false,
              dataType : "html",
              error    : $.proxy(function() { this.error("The requested content cannot be loaded. Please try after some seconds.") }, this),
	...
	lightBoxClose: function() {
      return $.LightBoxObject.hideRex();
    },

	moveBoxInline: function() {
      return $.LightBoxObject.resizeRex();
    }
...
$jQueryconflict(document).ready(function(){
	$jQueryconflict('.lightboxixcba').lightbox();
});
...
class uBillboard {
	...
	private function thumbnails($slides)
	{
		...
		$i = 0;
		//$out = "<script type=\"text/javascript\">\njQuery(document).ready(function(){ jQuery('.bxslider').bxSlider({ controls: true, auto: true, autoStart: false, nextSelector: '#slider-next', prevSelector: '#slider-prev', nextText: '', prevText: '', speed: 3000, buildPager: function(slideIndex){ switch(slideIndex){ \n";
		$out = "<ul class=\"thumbSlider\">";
		foreach($slides as $slide) {
			$out .= "<li slideindex=\"{$i}\">";
